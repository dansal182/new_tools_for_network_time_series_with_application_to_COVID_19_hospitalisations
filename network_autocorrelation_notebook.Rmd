...
\end{equation}
$$
We use the $\mathbf{fiveNet}$, included in the CRAN $\texttt{GNAR}$ package, network throughout this notebook for producing all relevant figures.
...
## 2.3 Producing alternative Corbit plots
...
```{r, echo=FALSE}
  library(glasso)
  source("/Users/danielsalnikov/Documents/PhD/code_scripts/network_autocorrelation/corbit_scripts/corrbit.R")
source("/Users/danielsalnikov/Documents/PhD/code_scripts/network_autocorrelation/corbit_scripts/nacf_r.R")
source("/Users/danielsalnikov/Documents/PhD/code_scripts/network_autocorrelation/corbit_scripts/nacf_dummy_models.R")
...
source("/Users/danielsalnikov/Documents/PhD/code_scripts/vs_code_editing/plot_corbit_line_segment.r")
source("/Users/danielsalnikov/Documents/PhD/code_scripts/network_autocorrelation/corbit_scripts/rectangular_corbit_plot.r")

gdp_data <- apply(gdpVTS, 2, function(x){x / sd(x[1:50], na.rm = TRUE)})
aux = gdp_data
aux[is.na(aux)] = 0


# covariance matrix (i.e., autocovariance matrix at lag zero)
S = cor(aux)
# graphical lasso for learning a network
a = glasso(S, rho=0.15, nobs=50)
b = a$wi
b[abs(b) > 0.0] = 1
diag(b) = 0
...
stages_list = get_k_stages_adjacency_tensor(as.matrix(net_result), 3)


# compute the weights matrix
W = distances(net_result)
W0 = normalise_weights(stages_list, max(W), W)

# max lag and max stage for the corbit plots
max_lag_nacf = 10
max_stage_nacf = max(W)
```
...
### 3.2.1 Comparison with VAR at highest growing peak (i.e., second-wave peak)
...
### 3.2.2 Three-step ahead squared prediction error comparison
...
### 3.3.1 Three-step ahead squared prediction error
